...
def customer_dashboard():
    ...
    contracten = Contract.query.filter_by(gebruiker_id=current_user.id).order_by(Contract.contract_begin_datum.desc()).limit(5).all()
...
def register():
    if request.method == 'POST':
        ...
        existing_user = User.query.filter_by(emailadres=emailadres).first()
        if existing_user:
        ...
        # Create new user (default role is 'klant')
        new_user = User(
            naam=naam,
